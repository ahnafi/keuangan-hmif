                ...
                <div class="p-1.5 min-w-full inline-block align-middle">
                    <div
                                ...
                                @forelse($funds as $fund)
                                    <tr>
                                        <td class="size-px whitespace-nowrap">
                                            <div class="ps-6 py-3">
                                                <span class="text-sm text-gray-800 dark:text-neutral-200">{{ $fund->id }}</span>
                                            </div>
                                        </td>
                                        <td class="h-px w-72 whitespace-nowrap">
                                            <div class="px-6 py-3">
                                                <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $fund->name }}</span>
                                            </div>
                                        </td>
                                        <td class="size-px whitespace-nowrap">
                                            <div class="px-6 py-3">
                                                <span class="text-sm text-gray-500 dark:text-neutral-500">{{ $fund->created_at->format('d M Y, H:i') }}</span>
                                            </div>
                                        </td>
                                        <td class="size-px whitespace-nowrap">
                                            <div class="px-6 py-1.5 flex gap-x-2">
                                                ...
                                                <a class="inline-flex items-center gap-x-1 text-sm text-green-600 decoration-2 hover:underline focus:outline-hidden focus:underline font-medium dark:text-green-500"
                                                    href="{{ route('fund.edit', $fund) }}">
                                                    Edit
                                                </a>
                                                <form method="POST" action="{{ route('fund.destroy', $fund) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus dana ini?')"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center">
                                            <p class="text-gray-500 dark:text-neutral-500">Tidak ada dana ditemukan.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                        ...
                        <div
                            ...
                            <div>
                                <p class="text-sm text-gray-600 dark:text-neutral-400">
                                    <span class="font-semibold text-gray-800 dark:text-neutral-200">{{ $funds->total() }}</span> hasil
                                </p>
                            </div>
